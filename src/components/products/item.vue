<template>
  <dl class="s-item">
    <dt><img :src="meta.image" alt="图片"></dt>
    <dd>
      <h3>
        <!-- 这里需要点击跳转产品详情页 则使用 router-link -->
      ...
      <p>
        <span class="s-item-tab">
          <span v-for="(v, i) in meta.tab" :key="v">{{ v + (meta.tab.length - 1 > i? '|': '')}}</span>
        </span>
      ...
      <ul class="deal-items">
        ...
        <li class="items" v-for="(item, index) in meta.dealItems.slice(0, 2)" :key="index">
          ...
          <p>
            ...
            <span class="deal-old-price">￥{{item.counterPrice}}</span>
            <span class="deal-sales">已售{{item.saleNum}}{{item.priceType}}</span>
          </p>
        </li>
      </ul>
    </dd>
