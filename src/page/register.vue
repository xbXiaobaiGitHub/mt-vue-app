  ...
  methods: {
    submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        // valid 代表的是你当前数据都认证成功了 才会走if成功后的流程
        if (valid) {
          api.register({
    ...
    input () {
      ...
      let regNum = /(\d)+/g
      let reg = /_/g
      let strongth = this.registerForm.password.match(reg) && this.registerForm.password.match(regNum) && this.registerForm.password.match(regStr)
      if (this.registerForm.password.length > 20 || (this.registerForm.password > 6 && strongth)) {
